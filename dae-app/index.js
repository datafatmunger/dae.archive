(function () {
  ...
  function showUpload() {
    ...
    document
      .querySelector("main nav li.upload")
      .addEventListener("click", async (e) => {
        await showSearch();
      });
    document.querySelector("main nav li.login").classList.add("hidden");
    document
      .querySelector("main .upload button")
      .addEventListener("click", async (e) => {
        await upload();
      });
    document
      .querySelector("main .upload #note")
      .addEventListener("keyup", async (e) => {
        if (e.keyCode === 13) await upload();
      });
  }
  ...
  function showSearch() {
    ...
    document
      .querySelector("main .search button.go")
      .addEventListener("click", async (e) => {
        doSearch();
      });
    document
  ...
  function showSorting() {
    ...
    //console.log("visible?")
    document.querySelector("#query").innerHTML = document.querySelector(
  ...
  function showMenu() {
    ...
    document
      .querySelector("main nav li.archive a")
      .setAttribute("href", `${url}/${username}`);
  ...
  function showResults(res) {
    ...
    document
      .querySelectorAll("main .search .results .res")
      .forEach((n) => n.remove());
  ...
  async function pickSortCategory(sortCategory) {
    ...
    let sortinput =
      sortCategory === "type"
        ? "&sort=ext+asc"
        : "&sort=" + sortCategory + (direction ? "+asc" : "+desc");
    let srt = sortinput;
    const res = await search(txt, srt);
    showResults(res);
  ...
  async function upload() {
    const note = document.querySelector('main .upload input[name="note"]')
      .value;
    ...
    try {
      ...
      showMsg(
        "Success! Your file has been uploaded and will be indexed and searchable after 02:05am tomorrow."
      );
    } catch (error) {
      showMsg("Fail.", true);
    }
  ...
  async function search(txt, srt) {
    const res = await fetch(`${url}/solr?q=${txt}&rows=300${srt}`, {
      credentials: "same-origin",
    });
    return await res.json();
  }
  ...
  async function login() {
    const email = document.querySelector('main .login input[name="email"]')
      .value;
    const pass = document.querySelector('main .login input[name="password"]')
      .value;
